...
#Encuesta Privada A
encuestaA_online <- read_csv("encuestaA_online.csv")[-2,] %>% 
...
# Encuesta Privada B
encuestaB_Privada_online <- read_csv("encuestaB_Privada_online.csv") %>% 
  mutate(fecha = lubridate::as_date(EndDate, format = "%Y-%m-%d"),
         encuesta = "B_Privada",
...
# Encuesta Pública B
encuestaB_Publica_online <- read_csv("encuestaB_Publica_online.csv") %>% 
  mutate(fecha = lubridate::as_date(EndDate, format = "%Y-%m-%d"),
         encuesta = "B_Publica",
         uemail = as.numeric(uemail),
         total_reward = as.numeric(total_reward))  %>%
...
#Encuesta Privada C
encuestaC_Privada_online <- read_csv("encuestaC_Privada_online.csv") %>% 
  mutate(fecha = lubridate::as_date(EndDate, format = "%Y-%m-%d"),
         encuesta = "C_Privada",
         uemail = as.numeric(uemail), 
...
#Encuesta Publica C
encuestaC_Publica_online <- read_csv("encuestaC_Publica_online.csv") %>% 
  mutate(fecha = lubridate::as_date(EndDate, format = "%Y-%m-%d"),
         encuesta = "C_Publica",
         uemail = as.numeric(uemail),
...
```{r}
# Preguntas de compresión Públicas
...
save(encuestas, file = "Pilot_data.Rdata")
