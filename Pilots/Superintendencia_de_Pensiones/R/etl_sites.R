...
sitios <-
  ...
  unite("inicio_wix", c("inicio_wix.p1...55", "inicio_wix.p1...56", "inicio_wix.p3...65", "inicio_wix.p3...66", "inicio_wix.p2", "inicio_wix.p4", "inicio_wix.p5", "inicio_wix.p6", "inicio_wix.p7", "inicio_wix.p8"), na.rm = TRUE, remove = TRUE)
...
contestar <- sitios2 %>%
  ...
  pivot_longer(cols=c(n_contestar.p1,n_contestar.p2,n_contestar.p3,n_contestar.p4,
                      n_contestar.p5, n_contestar.p8), values_to = "n_contestar") %>% 
  select(-name) %>% 
  distinct() %>%
  ...
  terminaron = "si") %>%
  group_by(useridn) %>%
  filter(contesta2 == max(contesta2))
contestar <- delete.na(contestar, 1) # delete rows with all NA values
