---
title: "Analysis"
author: "Joselina Davyt-Colo y Denise Laroze"
date: '2022-07-05'
...
```{r}
df <- readRDS(paste0(path_datos, "df.rds"))
sitios_sm <- readRDS(paste0(path_datos, "sitios_complete.rds"))
```


```{r}
zi <- df %>% filter(useridn != 0)  %>% group_by(useridn) %>% summarise(n = n())
```
...
# 
# n_clicks_wilcox  <- ggpubr::compare_means(n_clicks ~ Treatments,  data = df, 
...
```{r}
h <- round(mean(df$n_clicks, na.rm = TRUE), digits = 2)
...
```{r}
ggline(df, x = "Treatments", y = "n_clicks", add = "mean_se",
...
```{r}
ggpubr::compare_means(tiempo_sec_1click_nclick ~ Treatments,  data = df, ref.group = "Baseline",
              method = "t.test")
```


```{r}
h <- round(mean(df$tiempo_sec_1click_nclick, na.rm = TRUE), digits = 1)
...
```{r}
g <- df %>% ggplot(aes(tiempo_min_1click_nclick, n_clicks)) 
...
```{r, data}
dependent <- df %>%
  dplyr::select(correct_response, Confidence_1, Curiosity_1, InfoUtil_1, tiempo_min_1click_nclick, Treatments, Gender, Pension_Type) %>%
